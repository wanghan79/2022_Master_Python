...
class largeXMLDealer:
    ...
    def parse(self, fileName, func4Element=None ,*args):
        ...
        if 	len(self.ns )!=0 and len(args )!=0:
            for x in args:
                tags.append(self.ns+x)
            print(tags)
        else:
            tags =args


        context = etree.iterparse(fileName, events=["start"], tag=tags)
...
def Tree_elementlocal(elem,faterDict):
    if isinstance(elem, object):
        # print(elem)
        faterDict['child'] = []
        if len(elem):
            ...
            for child in elem:
                faterDict['child'].append({"root": child.tag})
                if len(child):
                    Tree_elementlocal(child,faterDict['child'][i] )
                i=i+1

if __name__ == "__main__":
    largeXMLDealer.parse( largeXMLDealer("P00734.xml"),"P00734.xml" ,Tree_elementlocal,"authorList")
